...
export default function AdminCategories(){
  ...
  async function add(){
    if(!name) return
    const r = await axios.post('/api/admin/categories',{ name })
    ...
    <main className="max-w-3xl mx-auto p-6">
      ...
      <div className="bg-white p-4 rounded shadow space-y-4">
        <div className="flex gap-3">
          <input placeholder="نام دسته" className="flex-1 border rounded px-3 py-2" value={name} onChange={e=>setName(e.target.value)} />
